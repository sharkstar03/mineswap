...
class LogController extends Controller
{
    ...
    public function investmentRunning()
    {
        $title = "Running Investment";
        $transaction = Investment::where('status',1)->with(['user','plan','miner'])->orderBy('id', 'DESC')->paginate(config('basic.paginate'));
        return view('admin.transaction.investment', compact('transaction','title'));
    }

    public function investmentComplete()
    {
        $title = "Complete Investment";
        $transaction = Investment::where('status',2)->with(['user','plan','miner'])->orderBy('id', 'DESC')->paginate(config('basic.paginate'));
        return view('admin.transaction.investment', compact('transaction','title'));
    }
    ...
    public function investmentSearch(Request $request)
    {
        ...
        $dateSearch = $request->datetrx;
        $date = preg_match("/^[0-9]{2,4}\-[0-9]{1,2}\-[0-9]{1,2}$/", $dateSearch);
        $transaction = Investment::where('status','!=',0)->with(['user','plan','miner'])->orderBy('id', 'DESC')
            ->when(isset($search['transaction']), function ($query) use ($search) {
                return $query->where('transaction', 'LIKE', "%{$search['transaction']}%");
            })
            ->when(isset($search['user_name']), function ($query) use ($search) {
                return $query->whereHas('user', function ($q) use ($search) {
                    $q->where('email', 'LIKE', "%{$search['user_name']}%")
