                ...
                @forelse($transaction as $k => $row)
                    <tr>
                        ...
                        <td data-label="@lang('Amount')">
                            <span class="cost text-{{($row->trx_type == "+") ? 'success': 'danger'}}">
                            {{$row->trx_type}}{{getAmount($row->amount, ($row->balance_type == config('basic.currency')) ? config('basic.fraction_number') : 8). ' ' . trans($row->balance_type)}}
                        </span>
                        </td>
