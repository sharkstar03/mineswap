...
class BasicController extends Controller
{
    ...
    public function updateConfigure(Request $request)
    {
        ...
        config(['basic.plan_sale_commission' => (int)$reqData['plan_sale_commission']]);
        ...
        config(['basic.bonus_amount' => (int)$reqData['bonus_amount']]);
    ...
    public function breadcrumbUpdate(Request $request)
    {
        ...
        if ($request->hasFile('form_thumbs')) {
            try {
                $old = 'form_thumbs.png';
                $this->uploadImage($request->form_thumbs, config('location.logo.path'), null, $old, null, $old);
            } catch (\Exception $exp) {
                return back()->with('error', 'Banner could not be uploaded.');
            }
        }
