...
class PaymentController extends Controller
{
    ...
    public function addFundRequest(Request $request)
    {
        ...
        $investment_id = null;
        if ($orderId != null) {
            $investment = Investment::where('transaction', $orderId)->where('user_id',auth()->user()->id)->where('status',0)->orderBy('id','desc')->firstOrFail();
            $reqAmount = $investment->price;
        ...
        $fund = $this->newFund($request, $user, $gate, $charge, $final_amo, $reqAmount ,$investment_id);
        ...
        return [
            ...
            'in_currency' => trans('In') . ' ' . $method_currency,
            'in_amount' =>  getAmount($fund->final_amount,2),
            'isCrypto' => $isCrypto,
     ...
     */
    public function newFund(Request $request, $user, $gate, $charge, $final_amo, $amount ,$investment_id = null): Fund
    {
        $fund = new Fund();
        $fund->user_id = $user->id;
        $fund->gateway_id = $gate->id;
        $fund->investment_id = $investment_id;
